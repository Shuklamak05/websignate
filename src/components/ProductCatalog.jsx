...
const CATEGORY_CONFIG = [
  { id: 'dairy-cattle', label: 'Dairy Cattle', productCategory: 'ruminant' },
  { id: 'companion-animals', label: 'Companion Animals', productCategory: 'companion' },
...
function ShowMoreCard({ categoryId, label }) {
  return (
    ...
    >
      <h4 style={{ fontSize: '1.1rem', color: 'var(--color-forest)', fontWeight: 700, textAlign: 'center' }}>
  ...
  return (
    <section id="catalog" className="section-padding" style={{ backgroundColor: 'var(--color-ivory)' }}>
      <div className="container">
        ...
        {!isCategoryPage && CATEGORY_CONFIG.map((category) => {
          const data = category.productCategory ? searchableProducts.filter((prod) => prod.category === category.productCategory) : [];
          return (
            <div key={category.id} style={{ marginBottom: '3.5rem' }}>
              <h3 style={{ fontSize: '1.65rem', color: 'var(--color-forest)', marginBottom: '1.2rem' }}>{category.label}</h3>
              {category.id === 'poultry' ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '2rem' }} className="product-grid">
                  ...
                  {data.length > 5 && <ShowMoreCard categoryId={category.id} label={category.label} />}
                </div>
              )}
            </div>
          );
          ...
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(1, 42, 28, 0.4)', backdropFilter: 'blur(8px)', zIndex: 2000, display: 'flex', justifyContent: 'flex-end' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', color: selectedProduct.color, backgroundColor: `${selectedProduct.color}15`, padding: '0.35rem 0.8rem', borderRadius: '100px', letterSpacing: '1px' }}>
                  {selectedProduct.category} formulation
                </span>
                <button onClick={() => setSelectedProductId(null)} style={{ border: 'none', background: 'none', cursor: 'pointer', width: '36px', height: '36px', borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-forest)' }}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', marginBottom: '1.75rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {selectedProduct.ingredients.map((ing) => (
                    <div key={ing} style={{ padding: '0.75rem 1rem', borderRadius: '8px', backgroundColor: 'rgba(1, 42, 28, 0.02)', border: '1px solid rgba(1, 42, 28, 0.05)', fontSize: '0.85rem', fontWeight: 600, color: 'var(--color-forest)' }}>
                      {ing}
                    </div>
                  ))}
                </div>
              ...
              <div style={{ marginTop: 'auto', backgroundColor: 'var(--color-sage)', padding: '1.25rem 1.5rem', borderRadius: '16px', border: '1px solid rgba(1, 42, 28, 0.08)', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              ...
              <a href="#contact" onClick={() => setSelectedProductId(null)} className="btn-primary" style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', backgroundColor: 'var(--color-forest)', color: 'var(--color-white)', boxShadow: 'none' }}>
